...
function extractTrecenaFromFilename(filename) {
  const match = filename.match(/^meditations-(.+)\.json$/);
  if (!match) {
    throw new Error(`Invalid filename format: ${filename}. Expected format: meditations-*.json`);
  }
  return match[1];
}

async function processMeditationFile(pool, filePath) {
  ...
  for (const dayData of jsonData.days) {
    try {
      ...
      const meditation = dayData.meditation || null;
      if (!meditation) {
        console.log(`  - Day ${day}: nothing to insert (no meditation field)`);
        continue;
      }
      ...
      await pool.query(
        `UPDATE days SET meditation = $1, updated_at = NOW() WHERE id = $2`,
        [meditation, dayId]
      );

      successCount++;
      console.log(`  ✓ Day ${day}: meditation=${meditation.length} chars`);
    } catch (error) {
...
async function main() {
  console.log('Starting meditation data insertion...\n');
  ...
  const files = fs
    ...
    .filter((file) => file.startsWith('meditations-') && file.endsWith('.json'))
    .map((file) => path.join(INPUT_DIR, file))
    .sort();

  if (files.length === 0) {
    console.log('No meditations-*.json files found in input directory.');
    return;
  }
  ...
  try {
    ...
    for (const filePath of files) {
      try {
        const result = await processMeditationFile(pool, filePath);
        results.push(result);
