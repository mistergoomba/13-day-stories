...
function extractTrecenaFromFilename(filename) {
  const match = filename.match(/^horoscopes-(.+)\.json$/);
  if (!match) {
    throw new Error(`Invalid filename format: ${filename}. Expected format: horoscopes-*.json`);
  }
  return match[1];
}

async function processHoroscopeFile(pool, filePath) {
  ...
  for (const dayData of jsonData.days) {
    try {
      ...
      const horoscope = dayData.horoscope || null;
      const horoscopePrompt = dayData.image_prompts?.horoscope || null;

      if (!horoscope && !horoscopePrompt) {
        console.log(`  - Day ${day}: nothing to insert (no horoscope or image_prompt.horoscope)`);
        continue;
      }
      ...
      if (horoscope) {
        await pool.query(
          `UPDATE days SET horoscope = $1, updated_at = NOW() WHERE id = $2`,
          [horoscope, dayId]
        );
      }
      ...
      if (horoscopePrompt) {
        await pool.query(
          `INSERT INTO image_prompts (day_id, horoscope, updated_at)
           VALUES ($1, $2, NOW())
           ON CONFLICT (day_id)
           DO UPDATE SET
             horoscope = EXCLUDED.horoscope,
             updated_at = NOW()`,
          [dayId, horoscopePrompt]
        );
      }

      successCount++;
      console.log(`  ✓ Day ${day}: horoscope=${horoscope ? horoscope.length + ' chars' : 'skip'}, prompt=${horoscopePrompt ? 'yes' : 'skip'}`);
    } catch (error) {
...
async function main() {
  console.log('Starting horoscope data insertion...\n');
  ...
  const files = fs
    ...
    .filter((file) => file.startsWith('horoscopes-') && file.endsWith('.json'))
    .map((file) => path.join(INPUT_DIR, file))
    .sort();

  if (files.length === 0) {
    console.log('No horoscopes-*.json files found in input directory.');
    return;
  }
  ...
  try {
    ...
    for (const filePath of files) {
      try {
        const result = await processHoroscopeFile(pool, filePath);
        results.push(result);
